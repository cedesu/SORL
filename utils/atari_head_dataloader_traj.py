...
class AtariHeadDataloader():
    def __init__(self, directory, batch_size=32, stack=3, controls=18,
        ...
        train_ratio=0.9
        self.traj_len_train=self.traj_len[:int(len(self.traj_len)*train_ratio)]
        self.traj_len_eval=self.traj_len[int(len(self.traj_len)*train_ratio):]
        self.total_len_train = sum(self.traj_len_train)
        self.total_len_eval=sum(self.traj_len_eval)
    ...
    def get_batch(self, samples):
        ...
        traj, _ = self._get_index_traj_and_sample(samples[0])
        for ix in list(np.random.choice(self.traj_len[traj],self.batch_size,replace=False)):
...
class MultiprocessAtariHeadDataLoader():
    ...
    def __init__(self, dataloader_args, workers):
        ...
        self.sample_length_train = loader.total_len_train # Total number of samples
        self.sample_length_eval = loader.total_len_eval # Total number of samples
    ...
    def __iter__(self):
        ...
        # if self.is_train:
        #     samples = list(range(self.sample_length_train))
        # else:
        #     samples = list(np.arange(self.sample_length_eval)+self.sample_length_train)
        # shuffle(samples)
        
        #not shuffle eval, eval=train
        # Generate random sequence of samples
        if self.is_train:
            samples = list(range(self.sample_length_train))
            shuffle(samples)
        ...
        # Fill request queue
        self.length=len(samples)//self.batch_size
    ...
    def switch_mode(self,is_train=True):
